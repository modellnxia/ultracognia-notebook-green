...
- ✅ **Peças de composição do layout `infographic` (guardrails) + provider de imagem OpenAI (2026-08-21)** — dois pedidos do usuário na mesma sessão.
  - **Guardrails de composição**: 59 slides de referência (3 PDFs reais gerados pelo NotebookLM, fornecidos pelo usuário) catalogados um a um — técnica documentada em `FEWSHOT_ANALYSIS_METHOD.md`, catálogos completos em `FEWSHOT_CATALOG_SHOT{1,2,3}.md`, destilado num vocabulário fechado em `FEWSHOT_RULEBOOK.md`. `Slide` ganhou 5 campos opcionais (`illustration_position`, `eyebrow_style`, `panel_style`, `arrangement`, `citation_style`) — a IA compõe combinações novas, mas só com peças desse vocabulário fechado, nunca inventa uma peça fora dele. `Theme.palette.background` passou a ser validado por luminância (rejeita fundo claro/branco — nenhum dos 59 exemplos reais tem isso — mas aceita qualquer tom escuro que a IA escolher, não trava num valor único). Escopo desta fatia: só o layout `infographic`; arranjos mais complexos do rulebook (`flow-arrows`/`table`/`radial`/`hierarchy`) documentados mas não implementados ainda.
  - **Pollinations removido, substituído pela API de imagens da OpenAI** (`gpt-image-1`, `DECK_IMAGE_PROVIDER=openai`) — decisão do usuário: Pollinations nunca foi pensado pra produção (sem SLA), e com a chave da OpenAI validada e com billing habilitado, não faz mais sentido mantê-lo. Qualidade máxima (`OPENAI_IMAGE_QUALITY=high`) por decisão explícita do usuário. Validado manualmente com a chave real antes de entrar em produção (imagem de teste gerada, baixada e conferida visualmente).
  - 213 testes automatizados no total do módulo.
